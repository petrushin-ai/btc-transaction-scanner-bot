...
export function classifyRedeemScript(script: Buffer): RedeemScriptType {
  ...
  if (
    script.length === SCRIPT_LENGTHS.P2WPKH_REDEEM
    && script[0] === OP.OP_0
    && script[1] === PUSH.BYTES_20
  ) return "p2wpkh";
  // P2WSH redeem: 0x00 0x20 <32>
  if (
    script.length === SCRIPT_LENGTHS.P2WSH_REDEEM
    && script[0] === OP.OP_0 && script[1] === PUSH.BYTES_32
  ) return "p2wsh";
  return "unknown";
}

// Taproot witness basics: classify a key-path vs. a script-path using witness stack layout
...
export function classifyTaprootWitnessBasic(witness: Buffer[]): TaprootWitnessKind {
  ...
  // where the control block length is 33 + 32*m with the first byte having bit 0 indicating
  // parity and top bits encoding a leaf version.
  // Minimal check: last element length >= 33 and first byte & 0x80 is set
  // (0xc0 typical for tapscript v0xc0)
