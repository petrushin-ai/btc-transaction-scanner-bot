...
import { resolveFixturesDir } from "@/shared/helpers";
...
function extractAddressesFromBlockHex(hex: string, network: Network): {
  txCount: number;
...
export async function updateAddressesFromLatestFixture(
  options?: { maxPerCategory?: number; addressesFile?: string }
) {
  ...
  if ( pairs.length === 0 ) {
    ...
    const result =
      spawnSync( "bun", [ "src/shared/scripts/fetch-block-fixtures.ts" ], { stdio: "inherit" } );
  ...
  const currentSet = new Set(
    current.outputs.map( (o) => o.address ).filter( Boolean ) as string[]
  );
  ...
  const labelLookup = new Map<string, string | undefined>(
    existing.map( (e) => [ e.address, e.label ] )
  );
