...
export class EventService {
  ...
  /** Returns current backlog depth (queued and in-flight) for a specific event type. */
  public getBacklogDepth(type: DomainEventType): number {
    return this.getDepth( type );
  }

  /** Wait until the backlog for a given type goes below a threshold (default: maxQueueSize/2). */
  ...
  private async dispatchLoop(type: DomainEventType): Promise<void> {
    const q = this.queues.get( type );
    if ( !q ) return;
    // Process until the queue is empty at the moment; new items will schedule another tick
    while ( q.length > 0 ) {
