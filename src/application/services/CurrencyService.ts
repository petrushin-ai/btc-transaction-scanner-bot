...
export class CurrencyService implements CurrencyRateProvider {
  ...
  private negativeMemoryCache: Map<string, { errorMessage: string; statusCode?: number; cachedAtMs: number; ttlSeconds: number }> = new Map();
  ...
  private getCachedRate(base: CurrencyCode, quote: CurrencyCode, ttlSeconds: number): ExchangeRate | null {
  ...
  private getNegativeCached(base: CurrencyCode, quote: CurrencyCode): { errorMessage: string; statusCode?: number } | null {
    const key = this.getCacheKey(base, quote);
    const now = Date.now();
    const neg = this.negativeMemoryCache.get(key);
    if (neg) {
      const ageSeconds = Math.max(0, Math.floor((now - neg.cachedAtMs) / 1000));
      if (ageSeconds <= neg.ttlSeconds) return { errorMessage: neg.errorMessage, statusCode: neg.statusCode };
    ...
    const ttlSeconds = Number.isFinite(entry.ttlSeconds) ? entry.ttlSeconds : this.getNegativeCacheTtlSeconds();
    ...
    if (Number.isFinite(cachedAtMs) && ageSeconds <= ttlSeconds) {
      this.negativeMemoryCache.set(key, {
        errorMessage: String(entry.errorMessage || entry.message || "rate temporarily unavailable"),
  ...
  private saveNegativeToCache(base: CurrencyCode, quote: CurrencyCode, errorMessage: string, statusCode?: number): void {
    const ttlSeconds = this.getNegativeCacheTtlSeconds();
    const key = this.getCacheKey(base, quote);
    this.negativeMemoryCache.set(key, { errorMessage, statusCode, cachedAtMs: Date.now(), ttlSeconds });
