...
export class FeatureFlagsService {
  ...
  private emit(): void {
    for (const l of this.listeners) {
      try { l({ ...this.current }); } catch { /* ignore */ }
    }
  }
      ...
      this.timer = setInterval(() => {
        try {
          ...
          const next: FeatureFlags = {
            parseRawBlocks: typeof json.parseRawBlocks === "boolean" ? json.parseRawBlocks : this.current.parseRawBlocks,
            resolveInputAddresses: typeof json.resolveInputAddresses === "boolean" ? json.resolveInputAddresses : this.current.resolveInputAddresses,
          };
          if (!deepEqualFlags(this.current, next)) {
            this.current = next;
            this.log.info({ type: "flags.updated", source: "file", filePath: absolute, flags: next });
            this.emit();
          }
        } catch (err) {
...
function deepEqualFlags(a: FeatureFlags, b: FeatureFlags): boolean {
  return a.parseRawBlocks === b.parseRawBlocks && a.resolveInputAddresses === b.resolveInputAddresses;
}
...
function normalizeSource(src?: FeatureFlagsSource): FeatureFlagsSource | undefined {
  ...
  const reloadIntervalMs = src?.reloadIntervalMs ?? (process.env.FEATURE_FLAGS_RELOAD_MS ? Number(process.env.FEATURE_FLAGS_RELOAD_MS) : undefined);
