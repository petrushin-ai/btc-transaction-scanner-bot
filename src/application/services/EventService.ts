...
export class EventService {
  ...
  private async runWithConcurrencyAndRetry(sub: InternalSubscription, ev: DomainEvent): Promise<void> {
    ...
    try {
      ...
      for (;;) {
        try {
          ...
          this.log.warn({ type: "event.handler.error", sub: sub.name, eventType: ev.type, attempt, message });
          if (attempt > sub.retry.maxRetries) {
            this.log.error({ type: "event.handler.failed", sub: sub.name, eventType: ev.type, attempts: attempt });
            return;
          }
