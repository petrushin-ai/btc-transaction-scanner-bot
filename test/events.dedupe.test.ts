  ...
  it( "carries dedupeKey through AddressActivityFound and NotificationEmitted", async () => {
    ...
    events.subscribe( {
      ...
      handler: (e
        ...
        if ( e.dedupeKey ) seen.push( e.dedupeKey );
      }
    } );
    // Minimal pipeline fragment: directly publish AddressActivityFound
    // and expect NotificationEmitted handler from the pipeline to set dedupe
    // Here we simulate the sink stage
    events.subscribe( {
