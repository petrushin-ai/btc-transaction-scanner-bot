...
// Minimal fakes for services used by a pipeline
...
describe( "Pipeline integration", () => {
  it(
    "flows BlockDetected -> BlockParsed -> AddressActivityFound -> NotificationEmitted",
    async () => {
      ...
      // Tap events
      events.subscribe( {
        event: "BlockParsed", handler: () => {
          sawParsed = true;
        }
      } );
      events.subscribe( {
        event: "AddressActivityFound", handler: () => {
          sawActivity += 1;
        }
      } );
      events.subscribe( {
        event: "NotificationEmitted", handler: () => {
          sawNotification += 1;
        }
      } );
