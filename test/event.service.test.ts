...
describe( "EventService", () => {
  ...
  it( "respects concurrency per subscription", async () => {
    ...
    for ( let i = 0; i < 5; i++ ) {
      await events.publish( {
        ...
        height: i,
        hash: String( i ),
        dedupeKey: `BlockDetected:${ i }:${ String( i ) }`
      } );
    }
    await wait( 150 );
  ...
  it( "exerts backpressure when queue is full", async () => {
    ...
    let handled = 0;
    events.subscribe( {
      event: "BlockDetected", handler: async () => {
        await wait( 30 );
        handled += 1;
      }
    } );
    ...
    await events.publish( {
      ...
      height: 1,
      hash: "h1",
      dedupeKey: "BlockDetected:1:h1"
    } );
    // the second publish should wait until the first drains
    // because queue size is 1 and handler is slow
    await events.publish( {
      type: "BlockDetected",
      timestamp: new Date().toISOString(),
      height: 2,
      hash: "h2",
      dedupeKey: "BlockDetected:2:h2"
    } );
