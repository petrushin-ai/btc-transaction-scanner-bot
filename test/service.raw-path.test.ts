...
describe( "Service raw parsing path", () => {
  test(
    "parseBlockByHash with parseRawBlocks=true returns expected tx count and metadata",
    async () => {
      ...
      const anyAddress = block.transactions
        .some( (t) => t.outputs.some( (o) => Boolean( o.address ) ) );
