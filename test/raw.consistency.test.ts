...
describe( "Raw block parsing consistency", () => {
  test( "tx count matches verbose JSON fixture and decodes addresses/OP_RETURN", () => {
    ...
    const entries = fs.readdirSync( fixturesDir )
      .filter( (f) => f.endsWith( "-current" + ".raw" ) );
    ...
    const addressCount = parsed.transactions
      .reduce( (acc, t) =>
        acc + t.outputs.filter( (o) =>
          Boolean( o.address ) ).length, 0 );
    expect( addressCount ).toBeGreaterThan( 0 );

    const opRetCount = parsed.transactions
      .reduce( (acc, t) =>
        acc + t.outputs.filter( (o) =>
          o.scriptType === "nulldata" ).length, 0 );
    // zero is possible but unlikely; still assert that opReturn entries, if any, carry data hex
    if ( opRetCount > 0 ) {
      const withData = parsed.transactions
        .flatMap( (t) =>
          t.outputs ).filter( (o) =>
          o.scriptType === "nulldata" && Boolean( o.opReturnDataHex ) );
