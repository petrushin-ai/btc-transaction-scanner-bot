...
function loadLatestParsedBlock(): any {
  ...
  const json = JSON.parse(fs.readFileSync(path.join(fixturesDir, entries[0]), "utf8"));
  // Shape this into ParsedBlock via BitcoinService.parseTransactions
  return json;
}
...
describe("Transaction matching performance", () => {
  test("checkTransactions with 1000 addresses under time budget", async () => {
    ...
    emitMetric({ suite: "matching", name: "check_1000_addresses_ms", value: Math.round(t1 - t0), unit: "ms", details: { activities: activities.length } });
